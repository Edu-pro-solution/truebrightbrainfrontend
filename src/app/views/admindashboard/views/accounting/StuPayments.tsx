...
export default function PaymentHistory() {
  ...
  const listUrl = currentSession?._id
    ? `/receipt-session/${currentSession._id}`
    : null;
  ...
  const receiptUrl =
    view === "receipt" && selectedPayment?._id
  ...
  const school = useMemo(() => {
    const settings = Array.isArray(schoolSettings)
      ? schoolSettings[0]
      : schoolSettings;
    const profile = Array.isArray(schoolProfile)
      ? schoolProfile[0]
      : schoolProfile;
    const rawLogo =
      settings?.schoolLogo || settings?.logo || settings?.logoUrl || "";
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put(
        `${apiUrl}/api/receipt/${selectedPayment._id}`,
        {
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      await axios.delete(`${apiUrl}/api/receipt/${selectedPayment._id}`, {
        headers: authHeaders(),
      });
        ...
        <FormShell
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              value={editForm.paid}
              onChange={(e) =>
                setEditForm((f) => ({ ...f, paid: e.target.value }))
              }
            />
          </div>
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="e.g. Complete / Partial"
            />
          </div>
          <div className="space-y-2">
            <Label className="text-[10px] font-bold uppercase text-slate-400">
  ...
  if (view === "receipt" && selectedPayment && receiptDetail) {
    ...
    const studentName = String(
      receiptDetail.studentName ?? receiptDetail.name ?? "—",
    );
    ...
    const receiptNo = String(
      receiptDetail.receiptNo ??
        receiptDetail.serial ??
        `RCPT-${String(receiptDetail._id ?? "").slice(-8)}`,
    );
          ...
          <CardHeader className="bg-card border-b space-y-3">
            ...
            <div>
              ...
              <p className="text-sm text-black">
                {[school.address, school.email].filter(Boolean).join(" • ")}
              </p>
              <p className="text-sm text-slate-500">
                14, Babs Ladipo Street, Lagos • support@edana.com
              </p>
            </div>
          </CardHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="border-b border-border p-4">
                ...
                <p className="font-semibold text-foreground">
                  {Number.isNaN(paid)
                    ? String(receiptDetail.paid ?? "—")
                    : `N ${paid.toLocaleString()}`}
                </p>
              </div>
              <div className="border-b border-border p-4">
                <p className="text-xs font-bold uppercase text-muted-foreground">
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Track student financial records and outstanding balances.
        </p>
      </div>
        ...
        <CardContent className="p-0">
          <Table>
            <TableHeader className="bg-primary/10">
              <TableRow>
                <TableHead className="w-[60px] text-primary font-bold pl-6 text-xs uppercase">
                  S/N
                </TableHead>
                <TableHead className="text-primary font-bold text-xs uppercase">
                  Student Name
                </TableHead>
                <TableHead className="text-primary font-bold text-xs uppercase">
                  Total Fees
                ...
                <TableHead className="text-primary font-bold text-xs uppercase text-center">
                  Balance
                </TableHead>
                <TableHead className="text-primary font-bold text-xs uppercase">
                  Status
                </TableHead>
                <TableHead className="text-right pr-6 text-primary font-bold text-xs uppercase">
                  Action
                </TableHead>
              </TableRow>
            </TableHeader>
              ...
              {listLoading ? (
                ...
                <TableRow>
                  <TableCell
                    colSpan={9}
                    className="py-12 text-center text-muted-foreground">
                    No payment records for this session.
                  </TableCell>
                </TableRow>
              ) : (
                currentPayments.map((item, index) => {
                  ...
                  const studentId = String(
                    item.studentId ?? item.student ?? item.student_id ?? "",
                  );
                  const dateStr = item.date
                    ? new Date(
                        item.date as string | number,
                      ).toLocaleDateString()
                    : "—";
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-52">
                            ...
                            <DropdownMenuItem
                              ...
                              <Pencil size={14} className="text-[#180154]" />{" "}
                              Edit
                            </DropdownMenuItem>
                            <DropdownMenuItem
