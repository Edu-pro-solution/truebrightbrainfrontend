...
export default function NoticeBoard() {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (view === "add") {
        ...
        await axios.put(
          `${apiUrl}/api/edit-notice/${selectedNotice._id}`,
          {
            posted_by: form.postedBy,
            date: form.date,
            notice: form.message,
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      await axios.delete(`${apiUrl}/api/delete-notice/${selectedNotice._id}`, {
        headers: authHeaders(),
      });
      await reFetch();
  ...
  return (
    <div className="space-y-6 p-4">
      <div className="flex flex-row gap-4 justify-between items-center">
        <h2 className="text-2xl font-bold text-[#180154]">Notice Board</h2>
        <Button
          onClick={() => {
            setForm({ postedBy: "", date: "", message: "" });
            setSelectedNotice(null);
