...
function PsychoStudentReport() {
  ...
  const { data: examsRaw } = useFetch(
    sessionId ? `/getofflineexam/${sessionId}` : null,
  );
  const exams = useMemo(
    () => (Array.isArray(examsRaw) ? (examsRaw as any[]) : []),
    [examsRaw],
  );
  ...
  const handleGetStudents = async () => {
    ...
    try {
      const [stuRes, psyRes] = await Promise.allSettled([
        axios.get(`${apiUrl}/api/students/${sessionId}/${selectedClass}`, {
          headers: authHeaders(),
        }),
        selectedExam
          ? axios.get(`${apiUrl}/api/get-all-psy/${selectedExam}`, {
              headers: authHeaders(),
            })
      ...
      const students =
        stuRes.status === "fulfilled"
          ? Array.isArray(stuRes.value.data?.data || stuRes.value.data)
            ? stuRes.value.data?.data || stuRes.value.data
            : []
          : [];

      const psyMarks: any[] =
        psyRes.status === "fulfilled" ? psyRes.value.data?.scores || [] : [];

      const mapped: PsyRow[] = students.map((s: any) => {
        const existing = psyMarks.find(
          (p: any) => String(p.studentId?._id || p.studentId) === String(s._id),
        );
        return {
  ...
  const handleSave = async () => {
    if (!sessionId || !selectedExam) {
      toast.error("Select an exam to save");
      return;
    }
    setSaving(true);
    try {
      ...
      const checkRes = await axios
        .get(`${apiUrl}/api/get-all-psy/${selectedExam}`, {
          headers: authHeaders(),
        })
        .catch(() => ({ data: { scores: [] } }));
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-bold text-[#180154]">
        ...
        <CardContent className="pt-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            ...
            <Button
              ...
              disabled={loading}
              className="bg-[#180154] hover:bg-[#180154]/90 w-fit px-8">
      ...
      {fetched && (
        <div className="space-y-4">
          <div>
            <h3 className="text-lg font-bold text-[#180154]">
            ...
            <div className="overflow-x-auto">
              <Table>
                <TableHeader className="bg-[#E8EBF3]">
                  <TableRow>
                    <TableHead className="w-[40px] text-[#180154] font-bold pl-4">
                      #
                    </TableHead>
                    ...
                    <TableHead className="text-[#180154] font-bold text-[11px] leading-tight">
                      Following
                      <br />
                      Instruction
                    </TableHead>
                    ...
                    <TableHead className="text-[#180154] font-bold text-[11px] leading-tight min-w-[140px]">
                      Teacher Remarks
                    </TableHead>
                    <TableHead className="text-[#180154] font-bold text-[11px] leading-tight min-w-[140px]">
                      Principal Remarks
          ...
          <div className="flex justify-end mt-4">
            <Button
              onClick={handleSave}
              disabled={saving}
              className="bg-[#180154] hover:bg-[#180154]/90 px-10">
