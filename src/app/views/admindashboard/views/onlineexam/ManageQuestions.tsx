...
export default function ManageQuestions({ exam, onBack: _onBack }: Props) {
  ...
  const loadQuestions = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.get(`${apiUrl}/api/questions/${examId}`, {
        headers: authHeaders(),
      });
  ...
  const canSubmit = useMemo(() => {
    ...
    if (form.type === "multiple_choice")
      return form.options.every((o) => o.trim()) && !!form.answer.trim();
    ...
    if (form.type === "multiple_choice") {
      return {
        ...
        options: form.options.map((o) => ({
          option: o,
          isCorrect: o === form.answer,
        })),
      };
    }
    if (form.type === "true_false")
      return { ...base, correctAnswer: form.answer };
    if (form.type === "fill_in_the_blanks")
      return { ...base, possibleAnswers: form.answer };
    if (form.type === "theory")
      return { ...base, onscreenMarking: form.answer };
    return base;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await axios.put(
          `${apiUrl}/api/questions/${editingId}`,
          buildPayload(),
          { headers: authHeaders() },
        );
        ...
        await axios.post(
          `${apiUrl}/api/questions/${sessionId}`,
          buildPayload(),
          { headers: authHeaders() },
        );
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      await axios.delete(`${apiUrl}/api/questions/${deleteTarget._id}`, {
        headers: authHeaders(),
      });
  ...
  const passPercentage =
    exam?.percent ?? exam?.passPercentage ?? exam?.passMark ?? "—";
  ...
  return (
    <div className="p-6 space-y-6">
      <Button
        ...
        variant="ghost"
        onClick={_onBack}
        className="gap-2 border border-black text-black hover:bg-[#180154] hover:text-white">
          ...
          <CardContent className="p-0">
            <Table>
              <TableHeader className="bg-[#E8EBF3]">
                <TableRow>
                  <TableHead className="pl-6 text-[#180154] font-bold">
                    #
                  </TableHead>
                ...
                {loading ? (
                  ...
                  questions.map((q, index) => (
                    <TableRow key={q._id} className="hover:bg-slate-50/50">
                      ...
                      <TableCell className="text-[#180154] font-medium">
                        <div className="space-y-1">
                          <p className="line-clamp-2">{q.questionTitle}</p>
                          {getDisplayAnswer(q) ? (
                            <p className="text-xs text-slate-400">
                              Answer: {getDisplayAnswer(q)}
                            </p>
                          ) : null}
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="pr-6">
                        <div className="flex justify-end gap-2">
                          <Button
                            ...
                            className="h-8 w-8 text-[#180154] hover:bg-[#180154]/10 hover:text-[#180154]">
                            <Pencil className="h-4 w-4 text-[#180154]" />
                          </Button>
                          <Button
                            variant="ghost"
                            size="icon"
            ...
            <CardContent className="p-0">
              <div className="grid grid-cols-2">
                {[
                  ...
                  <div
                    key={label}
                    className="border-b border-r border-slate-200 p-4 odd:bg-slate-50/40">
                    <p className="text-xs font-bold uppercase text-slate-400">
                      {label}
                    </p>
                    <p className="mt-1 text-sm font-semibold text-[#180154] line-clamp-2">
                      {value || "—"}
                    </p>
                  </div>
                ))}
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => handleTypeChange(v as QType)}>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    value={form.mark}
                    onChange={(e) =>
                      setForm((f) => ({ ...f, mark: e.target.value }))
                    }
                    placeholder="e.g. 1"
                  />
                </div>
                ...
                <div className="flex justify-end gap-3 pt-2">
                  ...
                  <Button
                    ...
                    className="bg-[#180154] hover:bg-[#180154]/90">
                    {saving
                      ? "Saving…"
                      : editingId
                        ? "Save Changes"
