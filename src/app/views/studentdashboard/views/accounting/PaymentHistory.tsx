...
export default function StudentPaymentHistory() {
  ...
  const receipts = useMemo(() => {
    ...
    const name = String(
      userInfo?.studentName || userInfo?.username || userInfo?.name || "",
    )
      .toLowerCase()
      .trim();
    ...
    return data.filter((r: any) => {
      ...
      const rName = String(r.studentName || "")
        .toLowerCase()
        .trim();
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-[#180154]">Payment History</h2>
      ...
      <Card className="border-none shadow-sm ring-1 ring-slate-200 overflow-hidden">
        <CardHeader className="bg-slate-50/50 border-b">
          <div className="flex items-center gap-2">
            <ReceiptText className="h-4 w-4 text-[#180154]" />
            <CardTitle className="text-sm font-bold text-[#180154]">
              {loading
                ? "Loading…"
                : `${receipts.length} Payment${receipts.length !== 1 ? "s" : ""} Found`}
            </CardTitle>
          </div>
        </CardHeader>
            ...
            <Table>
              <TableHeader className="bg-[#E8EBF3]">
                <TableRow>
                  ...
                  <TableHead className="font-bold text-[#180154] text-right">
                    Total (₦)
                  </TableHead>
                  <TableHead className="font-bold text-[#180154] text-right">
                    Paid (₦)
                    ...
                    return (
                      <TableRow key={r._id} className="hover:bg-slate-50/50">
                        ...
                        <TableCell className="font-mono text-xs text-slate-600">
                          {r.receiptNo || r.serial || "—"}
                        </TableCell>
                        <TableCell className="text-slate-700">
                          {r.reason || "—"}
                        </TableCell>
                        <TableCell className="text-right font-semibold text-[#180154]">
                        ...
                        <TableCell>
                          <span
                            className={`text-[10px] font-bold px-2 py-1 rounded-full ${statusColor(r.status)}`}>
                            {r.status || "—"}
                          </span>
                        </TableCell>
                      </TableRow>
